...
const totalSum = (input) => {
  // Solution code here...
  let result = input.reduce((number, current) => {
    return number.concat(current);
  });
  return result.reduce( (number, current) => 
  number + current, 0);
};
...
let findMaleAndFemale = (data) => {
  ...
  data.filter((z)=>{
      ...
      if(z.gender=="female"||z.gender=="male"){
        arr.push(z.name)
      }
  })
  return arr.join(" and ")
...
let shortTest = '200';
data.map( (x) => {
  x.height > shortTest ? shortTest = x.name : '';
});
return shortTest;
 };
