...
const mapTwoToThe = (arr) => {
  // Solution code here...
  const map = arr.map(num => Math.pow(2, num));
  return map;
...
const charCode = (arr) => {
  ...
  arr.map(element => {
    return localArr.push(element.charCodeAt());
  });
  return localArr;
};
...
const evenOdd = (arr) => {
  ...
  arr.map(ele => {
    if (ele % 2 === 0) {
      return localArr.push("even")
    } else if (ele % 2 === 1) {
      return localArr.push("odd")
    } else {
      return localArr.push("N/A")
    }
  });
  return localArr;
};
...
const extractAbilities = (arr) => {
  ...
  let localArr = [];
  arr.map(str => {
    localArr.push(str.ability.name);
  });
  return localArr;
};
...
const extractStats = (arr) => {
  ...
  let localArr = [];
  arr.map(str => {
    localArr.push({ name: str.stat.name, total: str.effort + str.baseStat });
  });
  return localArr;
};
