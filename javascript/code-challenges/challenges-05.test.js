...
const toLastNames = people => {
  // Solution code here...
  let fullName = people.map((str) => {
    return str = `${str.firstName} ${str.lastName}`
  });
  return fullName;
};
...
const addValues = (arr) => {
  ...
  const sum = arr.reduce(
    (prevVal, curVal) => prevVal + curVal, inlVal);
  return sum;
};
...
const addPurchases = (arr) => {
  ...
  const inlVal = 0;
const sum = arr.reduce(
  (prevVal, curVal) => prevVal + curVal.purchasePrice,inlVal);
