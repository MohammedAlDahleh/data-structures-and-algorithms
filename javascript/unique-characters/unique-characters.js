...
class Hashmap {
    ...
    checkAllelemntSameType(str){
        let newData=str.split(" ")
        newData.forEach((str)=>{
            let first=this.hashK(str);
            let secand=this.hashK(str.toLowerCase());
            if(first!==secand){
                return false
            }
        })
...
console.log(myhashmap.checkAllelemntSameType(str))
